...
$sql23="select  pname,total,Oct,Nov,Dece,Jan,Feb,Mar,Apr,May,Jun,Jul,Aug,Sep 
     ...
     public function actionIcu3Index() {
        $model = new Formmodel();
        $names="รายงานจำนวนผู้เสียชีวิตด้วยโรคหลัก(Priciple Dx Icd-10 A00-A419,R651,R572) "; 
        if($model->load(Yii::$app->request->post())){
               $date1 = $model->date1;
               $date2 = $model->date2;            
              return $this->redirect(['icu3_preview','name'=>$names,'d1'=>$date1,'d2'=>$date2]);
        }
            return $this -> render('/site/ipd-icu/icu2-index',['mText'=>$this->mText,'names'=>$names,'model' => $model]);
    }       
     public function actionIcu3_preview($name,$d1,$d2) {
       $names = $name;
       $date1 = $d1;$date2 = $d2;
       $sql1 = "select 'จำนวนผู้เสียชีวิตด้วยโรคหลัก(Priciple Dx Icd-10 A00-A419,R651,R572) ' as pname, count(*) as cc
                    from 
                          (
                             select id.icd10, id.diagtype, a.pdx, a.dx0 from ipt i 
                             left outer join iptdiag id on i.an=id.an 
                             left outer join an_stat a on i.an=a.an
                             where i.dchtype in ('08','09') and (id.icd10 between 'A400' and 'A419' or id.icd10 in ('R651','R572')) 
                             and i.dchdate between '{$date1}' and '{$date2}' group by id.an
                    ) as df";
        try {
        ...
        return $this->render('/site/ipd-icu/icu3-preview',['mText'=>$this->mText,'names'=>$names,
                 'dataProvider'=>$rawData, 'date1' =>$date1,'date2' =>$date2]);           
     }
    public function actionIcu4Index() {
        $model = new Formmodel();
        $names="รายงานจำนวนผู้ป่วย Admit และ ผู้เสียชีวิตด้วยโรค Leptospirosis (Priciple Dx Icd-10 A27,A270,A278,A279)"; 
        if($model->load(Yii::$app->request->post())){
               $date1 = $model->date1;
               $date2 = $model->date2;
                 $url="http://192.168.3.8/stimulrep/stimulsoft/index.php?stimulsoft_client_key=ViewerFx&stimulsoft_report_key=ipd_icu/";   
                 return $this->redirect($url.'icu4.mrt&d1='.$date1.'&d2='.$date2);  
        }
            return $this -> render('/site/ipd-icu/icu-index',['mText'=>$this->mText,'names'=>$names,'model' => $model]);
    }     
    public function actionIcu5Index() {
        $model = new Formmodel();
        $names=" รายงานจำนวนผู้ป่วย Admit และ ผู้เสียชีวิตด้วยโรค Gastroenteritis (Priciple Dx Icd-10 A09,A090,A099)"; 
        if($model->load(Yii::$app->request->post())){
               $date1 = $model->date1;
               $date2 = $model->date2;
                 $url="http://192.168.3.8/stimulrep/stimulsoft/index.php?stimulsoft_client_key=ViewerFx&stimulsoft_report_key=ipd_icu/";   
                 return $this->redirect($url.'icu5.mrt&d1='.$date1.'&d2='.$date2);  
        }
            return $this -> render('/site/ipd-icu/icu-index',['mText'=>$this->mText,'names'=>$names,'model' => $model]);
    }        
