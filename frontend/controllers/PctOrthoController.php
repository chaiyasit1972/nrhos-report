...
class PctOrthoController extends Controller
{
    public $mText = "ข้อมูลระดับทีม";
    public $sText = "PCT - ศัลยกรรมกระดูกและข้อ(ออร์โธปิดิกส์)";    
    ...
    public function actionIndex1() {
        $model = new Formmodel();        
        $names=" Closed Fracture"; 
        if($model->load(Yii::$app->request->post())){
               $date1 = $model->date1;
               $date2 = $model->date2;
            return $this->redirect(['ortho1_preview','name'=>$names,'d1'=>$date1,'d2'=>$date2]);
        }        
        return $this->render('/site/pct-ortho/ortho1-index',['names'=>$names,'mText'=>$this->mText,'sText'=>$this->sText, 'model' => $model]);
    }
    public function actionOrtho1_preview($name,$d1,$d2) {
       $names=$name;
        $date1=$d1;$date2=$d2; 
        $sql1="select 'จำนวนผู้ป่วย Closed Fracture ทั้งหมด' as pnames,count(*) cc from an_stat where dchdate between '{$date1}' and '{$date2}'
                       and pdx between 'S52' and 'S5259' and spclty in ('08');";
       try {
       ...
       try {
        ...
        $sql3="select 'จำนวนผู้ป่วย Closed Fracture เสียชีวิตทั้งหมด' as pnames,count(*) cc  from 
       ...
       try {
        ...
        $sql4="select 'จำนวนผู้ป่วย Closed Fracture ส่งต่อ ทั้งหมด' as pnames,count(*) cc  from 
       ...
       try {
         ...
         $sql5="select 'จำนวนผู้ป่วย Closed Fracture ที่ Re admit ภายใน 28 วัน' as pnames,count(*) cc from an_stat a 
                                       ...
                                       and a.pdx between 'S52' and 'S5259' and a.spclty in ('08');";   
       try {
        ...
        return $this->render('/site/pct-ortho/ortho1-preview',['names'=>$names,'mText'=>$this->mText,'date1'=>$date1,'date2'=>$date2,
    ...
    public function actionIndex2() {
        ...
        if($model->load(Yii::$app->request->post())){
               $date1 = $model->date1;
               $date2 = $model->date2;
               $url="http://192.168.3.8/stimulrep/stimulsoft/index.php?stimulsoft_client_key=ViewerFx&stimulsoft_report_key=pct-ortho/";   
               return $this->redirect($url.'ortho2.mrt&d1='.$date1.'&d2='.$date2);                
        }        
        return $this->render('/site/pct-ortho/ortho2-index',['names'=>$names,'mText'=>$this->mText, 'sText' => $this->sText, 'model' => $model]);
    }    
    public function actionIndex3() {
        ...
        if($model->load(Yii::$app->request->post())){
               ...
               $date2 = $model->date2;
               $url="http://192.168.3.8/stimulrep/stimulsoft/index.php?stimulsoft_client_key=ViewerFx&stimulsoft_report_key=pct-ortho/";   
               return $this->redirect($url.'ortho3.mrt&d1='.$date1.'&d2='.$date2);                
        }        
        return $this->render('/site/pct-ortho/ortho2-index',['names'=>$names,'mText'=>$this->mText, 'sText' => $this->sText, 'model' => $model]);
    }
}