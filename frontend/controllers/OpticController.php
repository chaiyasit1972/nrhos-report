...
class OpticController extends Controller
{
    ...
    public function actionIndex() {
        $names="งานห้องตรวจตา(จักษุ)"; 
         return $this -> render('/site/optic/index',['sText'=>$this->sText, 'mText'=>$this->mText,'names'=>$names]);
    } 
     public function actionOptic1Index() {
        ...
        if($model->load(Yii::$app->request->post())){
            ...
            return $this -> render('/site/optic/optic1-index',['sText'=>$this->sText,'mText'=>$this->mText,'names'=>$names,'model' => $model]);
    }    
    public function actionOptic1_preview($name,$d1,$d2) {
      ...
      public function actionOptic2Index() {
        ...
        if($model->load(Yii::$app->request->post())){
            ...
            return $this -> render('/site/optic/optic2-index',['sText'=>$this->sText,'mText'=>$this->mText,'names'=>$names,'model' => $model]);
    }  
      public function actionOptic3Index() {
        ...
        if($model->load(Yii::$app->request->post())){
            ...
            return $this -> render('/site/optic/optic3-index',['sText'=>$this->sText,'mText'=>$this->mText,'names'=>$names,'model' => $model]);
    }  
     public function actionOptic4Index() {
        $model = new Formmodel();
        $names="รายงานการรับการส่งต่อ(refer-in) ผู้ป่วยโรคตา "; 
        if($model->load(Yii::$app->request->post())){
               $date1 = $model->date1;
               $date2 = $model->date2;
            return $this->redirect(['optic4_preview','name'=>$names,'d1'=>$date1,'d2'=>$date2]);
        }
            return $this -> render('/site/optic/optic1-index',['sText'=>$this->sText,'mText'=>$this->mText,'names'=>$names,
                                        'model' => $model]);
    }       
    public function actionOptic4_preview($name,$d1,$d2) {
        $names=$name;
        $date1=$d1;$date2=$d2;    
        $sql1 = "select v.aid, r.hn, concat(p.pname,p.fname,' ',p.lname) pname, r.refer_date, r.pre_diagnosis, v.pdx, 
                    i.name diag, h.`name` hospital,if(substr(v.aid,1,4)='3104','ในเขต','นอกเขต') status from referin r 
                    inner join patient p on r.hn=p.hn
                    inner join vn_stat v on r.vn=v.vn
                    inner join hospcode h on r.refer_hospcode=h.hospcode
                    left outer join icd101 i on v.pdx=i.code
                    where v.spclty='07' and r.refer_date between '{$date1}' and '{$date2}';";
        $sql2 = "select v.aid, r.hn, concat(p.pname,p.fname,' ',p.lname) pname, r.refer_date, r.pre_diagnosis, v.pdx, 
                    i.name diag, h.`name` hospital,if(substr(v.aid,1,4)='3104','ในเขต','นอกเขต') status from referin r 
                    inner join patient p on r.hn=p.hn
                    inner join vn_stat v on r.vn=v.vn
                    inner join hospcode h on r.refer_hospcode=h.hospcode
                    left outer join icd101 i on v.pdx=i.code
                    where v.spclty='07' and concat(h.chwpart,h.amppart) = '3104' and r.refer_date between '{$date1}' and '{$date2}' ;"; 
        $sql3 = "select v.aid, r.hn, concat(p.pname,p.fname,' ',p.lname) pname, r.refer_date, r.pre_diagnosis, v.pdx, 
                    i.name diag, h.`name` hospital,if(substr(v.aid,1,4) ='3104','ในเขต','นอกเขต') status from referin r 
                    inner join patient p on r.hn=p.hn
                    inner join vn_stat v on r.vn=v.vn
                    inner join hospcode h on r.refer_hospcode=h.hospcode
        ...
        return $this -> render('/site/optic/optic4-preview',
                             [
                                    'dataProvider1' => $dataProvider1,
                                    'dataProvider2' => $dataProvider2,
                                    'dataProvider3' => $dataProvider3,                                 
