 ...
 */
    ...
    public function actionCancer5Index() {
        $model = new Formmodel();
        $names="รายงานอัตราตายจากมะเร็งตับ"; 
        if($model->load(Yii::$app->request->post())){
               $date1 = $model->date1;
               $date2 = $model->date2;
               $a1 = 'dbpop_'.substr(substr($date1,0,4)+543,2,2).substr($date1,5,2);
               $a2 = 'dbpop_'.substr(substr($date2,0,4)+543,2,2).substr($date2,5,2);
               $sql1 = "select * from $a1  where substr(PRIMARYPROVINCE,1,4) = '3104' ;";
               $sql2 = "select * from $a2  where substr(PRIMARYPROVINCE,1,4) = '3104' ;"; 
               $num1 = \Yii::$app->db3->createCommand($sql1)->query()->rowCount; 
               $num2 = \Yii::$app->db3->createCommand($sql2)->query()->rowCount;           
              $pop = round(($num1+$num2)/2);
                $url="http://192.168.3.8/stimulrep/stimulsoft/index.php?stimulsoft_client_key=ViewerFx&stimulsoft_report_key=cancer/";   
                return $this->redirect($url.'cancer5.mrt&d1='.$date1.'&d2='.$date2.'&db='.$pop);  
        }
            return $this -> render('/site/ipd-cancer/cancer-index',['mText'=>$this->mText,'names'=>$names,'model' => $model]);
    }     
    public function actionCancer6Index() {
        $model = new Formmodel();
        $names="รายงานอัตราตายจากมะเร็งปอด"; 
        if($model->load(Yii::$app->request->post())){
               $date1 = $model->date1;
               $date2 = $model->date2;
               $a1 = 'dbpop_'.substr(substr($date1,0,4)+543,2,2).substr($date1,5,2);
               ...
               $pop = round(($num1+$num2)/2);               
                 $url="http://192.168.3.8/stimulrep/stimulsoft/index.php?stimulsoft_client_key=ViewerFx&stimulsoft_report_key=cancer/";   
                 return $this->redirect($url.'cancer6.mrt&d1='.$date1.'&d2='.$date2.'&db='.$pop);  
        }
            return $this -> render('/site/ipd-cancer/cancer-index',['mText'=>$this->mText,'names'=>$names,'model' => $model]);
    }     
    public function actionCancer7Index() {
        $model = new Formmodel();
        $names="รายงานภาวะแทรกซ้อน โรคร่วม(D70)"; 
        if($model->load(Yii::$app->request->post())){
               $date1 = $model->date1;
               $date2 = $model->date2;
                 $url="http://192.168.3.8/stimulrep/stimulsoft/index.php?stimulsoft_client_key=ViewerFx&stimulsoft_report_key=cancer/";   
                 return $this->redirect($url.'cancer7.mrt&d1='.$date1.'&d2='.$date2);  

        }
            return $this -> render('/site/ipd-cancer/cancer-index',['mText'=>$this->mText,'names'=>$names,'model' => $model]);
    }         
    public function actionCancer8Index() {
        $model = new Formmodel();
        $names="รายงานภาวะแทรกซ้อน โรคร่วม(D70 และ R509)"; 
        if($model->load(Yii::$app->request->post())){
               $date1 = $model->date1;
               $date2 = $model->date2;
                 $url="http://192.168.3.8/stimulrep/stimulsoft/index.php?stimulsoft_client_key=ViewerFx&stimulsoft_report_key=cancer/";   
                 return $this->redirect($url.'cancer8.mrt&d1='.$date1.'&d2='.$date2);  
