...
class PctChildController extends Controller
{
    ...
    public function actionIndex1() {
        ...
        return $this->render('/site/pct-child/child1-index',['names'=>$names,'mText'=>$this->mText,'sText' => $this->sText, 'model' => $model]);
    }
   ...
   public function actionChild4_preview($name,$d1,$d2) {
     ...
     public function actionIndex5() {
        $model = new Formmodel();        
        $names=" Asthma(J45,J452-J455,J459)"; 
        if($model->load(Yii::$app->request->post())){
               $date1 = $model->date1;
               $date2 = $model->date2;
   ...
   public function actionChild5_preview($name,$d1,$d2) {
    ...
    public function actionIndex6() {
        ...
        if($model->load(Yii::$app->request->post())){
               $date1 = $model->date1;
               $date2 = $model->date2;
               $url="http://192.168.3.8/stimulrep/stimulsoft/index.php?stimulsoft_client_key=ViewerFx&stimulsoft_report_key=pct-child/";   
               return $this->redirect($url.'child6.mrt&d1='.$date1.'&d2='.$date2);                
        }        
        return $this->render('/site/pct-child/child6-index',['names'=>$names,'mText'=>$this->mText, 'sText' => $this->sText, 'model' => $model]);
    }    
    public function actionIndex7() {
        ...
        if($model->load(Yii::$app->request->post())){
               ...
               $date2 = $model->date2;
               $url="http://192.168.3.8/stimulrep/stimulsoft/index.php?stimulsoft_client_key=ViewerFx&stimulsoft_report_key=pct-child/";   
               return $this->redirect($url.'child7.mrt&d1='.$date1.'&d2='.$date2);                
        }        
        return $this->render('/site/pct-child/child6-index',['names'=>$names,'mText'=>$this->mText, 'sText' => $this->sText, 'model' => $model]);
    }    
