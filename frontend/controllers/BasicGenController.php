...
class BasicGenController extends Controller
{
    ...
    public function actionBasicGen2() {
        ...
        if($model->load(Yii::$app->request->post())){
               ...
               if($check==0){
                      ...
                      return $this->redirect($url.'person2_1.mrt&d1='.$date1);                            
               }else{
                      $age1 = $model->text1;
                      $age2 = $model->text2;
                      $url="http://192.168.3.8/stimulrep/stimulsoft/index.php?stimulsoft_client_key=ViewerFx&stimulsoft_report_key=basic/"; 
                      return $this->redirect($url.'person2_2.mrt&d1='.$date1.'&age1='.$age1.'&age2='.$age2);                        
               }
    ...
    public function actionBasicGen16_preview($name,$d1,$d2) {
        ...
        $sql1="select a.pdx,i.name diag,count(*) num,sum(a.admdate) total,(sum(a.admdate)/count(*)) rate from an_stat a 
              inner join icd101 i on i.code=a.pdx where a.dchdate between '{$date1}' and '{$date2}'  group by a.pdx order by rate desc limit 40;";
        try {
              $rawData = \Yii::$app->db1->createCommand($sql1)->queryAll();
            } catch (\yii\db\Exception $e) {
                  throw new \yii\web\ConflictHttpException('sql error');
    ...
    public function actionBasicGen20() {
        ...
        if($model->load(Yii::$app->request->post())){
               ...
               $date2 = $model->date2;   
               $type = Yii::$app->request->post('type');               
               $typeA=  explode(',', $type);$type_c=$typeA[0];$type_n=$typeA[1];               
               $sql1="select  IF(MONTH(o.dchdate)>=10,YEAR(o.dchdate)+1 ,YEAR(o.dchdate))+543 AS yrs from ipt o
                       where o.dchdate between '{$date1}' and '{$date2}' group by yrs;";
               $result = \Yii::$app->db1->createCommand($sql1)->queryAll();      
            ...
            //return $this->redirect(['basic-gen20_preview','name'=>$names,'d1'=>$date1,'d2'=>$date2,'t'=>$type]);       
                switch ($type_c) {
                    case 0:
                        ...
                        return $this->redirect($url.'gen-in10-1.mrt&d1='.$date1.'&d2='.$date2.'&yrs='.$yrs.'&yra='.$yra);  
                    break;
                    default:
                        $url="http://192.168.3.8/stimulrep/stimulsoft/index.php?stimulsoft_client_key=ViewerFx&stimulsoft_report_key=gen/";   
                        return $this->redirect($url.'gen-in10-2.mrt&d1='.$date1.'&d2='.$date2.'&yrs='.$yrs.'&yra='.$yra.'&tn='.$type_n.'&tc='.$type_c);                          
                    break;
            }                 
    ...
    public function actionBasicGen21() {
        ...
        if($model->load(Yii::$app->request->post())){
            ...
            //return $this->redirect(['basic-gen21_preview', 'name'=>$names, 'd1'=>$date1, 'd2'=>$date2]);
               $url="http://192.168.3.8/stimulrep/stimulsoft/index.php?stimulsoft_client_key=ViewerFx&stimulsoft_report_key=basic/"; 
                return $this->redirect($url.'gen-in-11.mrt&d1='.$date1.'&d2='.$date2);                    
        }
    ...
    public function actionBasicGen29() {
        ...
        if($model->load(Yii::$app->request->post())){
            ...
            return $this -> render('/site/basic-gen/basic-gen-25',['mText'=>$this->mText,'names'=>$names,'model'=>$model]);
    }        
    ...
    public function actionBasicGen32() {
        ...
        if($model->load(Yii::$app->request->post())){
            ...
            return $this -> render('/site/basic-gen/basic-gen-21',['mText'=>$this->mText,'names'=>$names,'model'=>$model]);
    }       
    ...
    public function actionBasicGen34() {
        $model = new Formmodel();        
        $names="รายงานจำนวนผู้รับบริการผู้ป่วยนอก ตรวจโรคทั่วไป(GP) และ ผู้ป่วย DM,HT (ในเขตรับผิดชอบ[20 ชุมชน + 14 หมู่] )";
        if($model->load(Yii::$app->request->post())){
               $date1 = $model->date1;
               $date2 = $model->date2;
               $url="http://192.168.3.8/stimulrep/stimulsoft/index.php?stimulsoft_client_key=ViewerFx&stimulsoft_report_key=basic/"; 
                return $this->redirect($url.'person20.mrt&d1='.$date1.'&d2='.$date2);                  
        }
            return $this -> render('/site/basic-gen/basic-gen-23',['mText'=>$this->mText,'names'=>$names,'model'=>$model]);
    }      
    ...
    /*
     * 
     /////////////////////////////////////////    ให้ไปเขียนใน GenController  /////////////////////////////////////////////////////////////////
     * 
     * --- รายงานข้อมูลพื้นฐานทั่ไป
     * --- รายงานข้อมูลพื้นฐานผู้ป่วยนอก
     * --- รายงานข้อมูลพื้นฐานผู้ป่วยใน
     * 
     */
}    
