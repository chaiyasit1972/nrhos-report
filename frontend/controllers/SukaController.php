...
class SukaController extends Controller
{
   ...
   public function actionSuka2_preview($name, $d1, $t, $v1) {
    ...
    public function actionSuka3Index() {
        $model = new Formmodel();             
        $names=" รายงานจำนวนผู้ที่้สัมผัสเชื้อพิษสุนัขบ้าได้รับการฉีดวัคซีน PCEC  ทั้งหมด ในเขตรับผิดชอบ";
        if($model->load(Yii::$app->request->post())){
               $date1 = $model->date1;
               $date2 = $model->date2;
               $url="http://192.168.3.8/stimulrep/stimulsoft/index.php?stimulsoft_client_key=ViewerFx&stimulsoft_report_key=suka/"; 
                return $this->redirect($url.'suka3.mrt&d1='.$date1.'&d2='.$date2);
        }   
            return $this -> render('/site/suka/suka1-index',['mText'=>$this->mText,'names'=>$names,'model'=>$model]);
    }      
    public function actionSuka4Index() {
        $model = new Formmodel();             
        $names=" รายงานจำนวนผู้ที่้สัมผัสเชื้อพิษสุนัขบ้าได้รับการฉีดวัคซีน PCEC  ทั้งหมด นอกเขตรับผิดชอบ";
        if($model->load(Yii::$app->request->post())){
               $date1 = $model->date1;
               $date2 = $model->date2;
               $url="http://192.168.3.8/stimulrep/stimulsoft/index.php?stimulsoft_client_key=ViewerFx&stimulsoft_report_key=suka/"; 
                return $this->redirect($url.'suka4.mrt&d1='.$date1.'&d2='.$date2);
        }   
            return $this -> render('/site/suka/suka1-index',['mText'=>$this->mText,'names'=>$names,'model'=>$model]);
    }       
