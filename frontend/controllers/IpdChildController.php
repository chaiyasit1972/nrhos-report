...
class IpdChildController extends Controller
{
    ...
    public function actionIndex() {
     ...
     public function actionChild3Index() {
        $model = new Formmodel();
        $names="รายงาน 20 อันดับโรคแรกผู้ป่วยใน ตึกสกลฯชั้น 2 (กุมารเวชกรรม)"; 
        if($model->load(Yii::$app->request->post())){
               $date1 = $model->date1;
               $date2 = $model->date2;
            $url="http://192.168.3.8/stimulrep/stimulsoft/index.php?stimulsoft_client_key=ViewerFx&stimulsoft_report_key=ipd_child/";   
     ...
     public function actionChild4Index() {
        $model = new Formmodel();
        $names="รายงานจำนวนผู้ป่วยใน ตึกสกลฯชั้น 2 (กุมารเวชกรรม) ห้องพิเศษ แยกสิทธิ์การรักษา"; 
        if($model->load(Yii::$app->request->post())){
               $date1 = $model->date1;
               $date2 = $model->date2;
            $url="http://192.168.3.8/stimulrep/stimulsoft/index.php?stimulsoft_client_key=ViewerFx&stimulsoft_report_key=ipd_child/";   
            return $this->redirect($url.'child4.mrt&d1='.$date1.'&d2='.$date2);                         
        }
            return $this -> render('/site/ipd-child/child1-index',['mText'=>$this->mText,'names'=>$names,'model' => $model]);
    }      
}    
