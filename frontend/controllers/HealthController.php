...
class HealthController extends Controller
{
    ...
    public function actionIndex() {
         ...
         return $this -> render('/site/health/index',['mText'=>$this->mText,'names'=>$names,'sText'=>$this->sText]);
    } 
     ...
     public function actionHealth5Index() {
        $model = new Formmodel();
        $names="รายงานแพทย์แผนไทย(เปรียบเทียบผู้รับบริการผู้ป่วยนอก)"; 
        if($model->load(Yii::$app->request->post())){
               $date1 = $model->date1;
               $date2 = $model->date2; 
                $url="http://192.168.3.8/stimulrep/stimulsoft/index.php?stimulsoft_client_key=ViewerFx&stimulsoft_report_key=health/";   
                return $this->redirect($url.'health5.mrt&d1='.$date1.'&d2='.$date2);                         
        }
            return $this -> render('/site/health/health1-index',['mText'=>$this->mText,'names'=>$names,'model' => $model,
                                         'sText' => $this->sText]);
    }     
     public function actionHealth6Index() {
        $model = new Formmodel();
        $names="รายงานแพทย์แผนไทยแยกการนวดรักษาและการฝังเข็ม"; 
        if($model->load(Yii::$app->request->post())){
               $date1 = $model->date1;
               $date2 = $model->date2; 
                $url="http://192.168.3.8/stimulrep/stimulsoft/index.php?stimulsoft_client_key=ViewerFx&stimulsoft_report_key=health/";   
                return $this->redirect($url.'health6.mrt&d1='.$date1.'&d2='.$date2);                         
        }
            return $this -> render('/site/health/health1-index',['mText'=>$this->mText,'names'=>$names,'model' => $model,
                                         'sText' => $this->sText]);
    }       
