...
class PctMedController extends Controller
{
    ...
    public function actionIndex1() {
        $model = new Formmodel();        
        $names=" โรค Stemi(I21-I214)"; 
        if($model->load(Yii::$app->request->post())){
               $date1 = $model->date1;
               $date2 = $model->date2;
            return $this->redirect(['med1_preview','name'=>$names,'d1'=>$date1,'d2'=>$date2]);
        }        
        return $this->render('/site/pct-med/med1-index',['names'=>$names,'mText'=>$this->mText, 'sText' => $this->sText,'model' => $model]);
    }
   ...
   public function actionMed3_preview($name,$d1,$d2) {
    ...
    public function actionIndex4() {
        $model = new Formmodel();        
        $names=" โรคความดันโลหิต Ht(I10-I15)"; 
        if($model->load(Yii::$app->request->post())){
               $date1 = $model->date1;
               $date2 = $model->date2;
    ...
    public function actionIndex7() {
        ...
        if($model->load(Yii::$app->request->post())){
               $date1 = $model->date1;
               $date2 = $model->date2;
               $url="http://192.168.3.8/stimulrep/stimulsoft/index.php?stimulsoft_client_key=ViewerFx&stimulsoft_report_key=pct-med/";   
               return $this->redirect($url.'med7.mrt&d1='.$date1.'&d2='.$date2);                
        }        
        return $this->render('/site/pct-med/med7-index',['names'=>$names,'mText'=>$this->mText, 'sText' => $this->sText, 'model' => $model]);
    }    
    public function actionIndex8() {
        ...
        if($model->load(Yii::$app->request->post())){
               ...
               $date2 = $model->date2;
               $url="http://192.168.3.8/stimulrep/stimulsoft/index.php?stimulsoft_client_key=ViewerFx&stimulsoft_report_key=pct-med/";   
               return $this->redirect($url.'med8.mrt&d1='.$date1.'&d2='.$date2);                
        }        
        return $this->render('/site/pct-med/med7-index',['names'=>$names,'mText'=>$this->mText, 'sText' => $this->sText, 'model' => $model]);
    }    
    public function actionIndex9() {
        $model = new Formmodel();        
        $names="รายงานจำนวนผู้รับบริการผู้ป่วยนอก-ใน สาขา/แผนก อายุรกรรม "; 
        if($model->load(Yii::$app->request->post())){
               $date1 = $model->date1;
               $date2 = $model->date2;
               $url="http://192.168.3.8/stimulrep/stimulsoft/index.php?stimulsoft_client_key=ViewerFx&stimulsoft_report_key=pct-med/";   
               return $this->redirect($url.'med9.mrt&d1='.$date1.'&d2='.$date2);                
        }        
        return $this->render('/site/pct-med/med7-index',['names'=>$names,'mText'=>$this->mText, 'sText' => $this->sText, 'model' => $model]);
    }        
