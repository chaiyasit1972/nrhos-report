...
class OpdController extends Controller
{
    ...
    public function actionOpd6Index() {
        $model = new Formmodel();               
        $names="รายงานจำนวนผู้รับบริการผู้ป่วยนอก (OPD) แยกตามห้องตรวจ ในวันเวลาราชการ (08.00 - 16.00 น.)"; 
        if($model->load(Yii::$app->request->post())){
               $date1 = $model->date1;
               $date2 = $model->date2;   
               $url="http://192.168.3.8/stimulrep/stimulsoft/index.php?stimulsoft_client_key=ViewerFx&stimulsoft_report_key=opd/"; 
                return $this->redirect($url.'opd6.mrt&d1='.$date1.'&d2='.$date2);
        }
            return $this -> render('/site/opd/opd6-index',['mText'=>$this->mText,'names'=>$names, 'model' => $model]);
    }       
    public function actionOpd7Index() {
        $model = new Formmodel();               
        $names="รายงานจำนวนผู้รับบริการผู้ป่วยนอก (OPD) ได้รับการตรวจและกลับบ้าน ตามช่วงเวลา"; 
        if($model->load(Yii::$app->request->post())){
               $date1 = $model->date1;
               $date2 = $model->date2;   
               $url="http://192.168.3.8/stimulrep/stimulsoft/index.php?stimulsoft_client_key=ViewerFx&stimulsoft_report_key=opd/"; 
                return $this->redirect($url.'opd7.mrt&d1='.$date1.'&d2='.$date2);
        }
            return $this -> render('/site/opd/opd6-index',['mText'=>$this->mText,'names'=>$names, 'model' => $model]);          
    }       
    public function actionOpd8Index() {
        $model = new Formmodel();               
        $names="รายงานจำนวนผู้รับบริการผู้ป่วยนอก แผนกจักษุ สิทธิ UC(ผู้สูงอายุ) ได้รับการตรวจคัดกรองตา"; 
        if($model->load(Yii::$app->request->post())){
               $date1 = $model->date1;
               $date2 = $model->date2;   
               $url="http://192.168.3.8/stimulrep/stimulsoft/index.php?stimulsoft_client_key=ViewerFx&stimulsoft_report_key=opd/"; 
                return $this->redirect($url.'opd8.mrt&d1='.$date1.'&d2='.$date2);
        }
            return $this -> render('/site/opd/opd6-index',['mText'=>$this->mText,'names'=>$names, 'model' => $model]);
    }        
