...
{
    public $mText = "งานศัลกรรมกระดูก(อาคาร 4 ชั้น 1)";
    ...
    public function actionPct1_preview($name, $d1, $d2) {
       ...
       try {
        ...
        $sql5 = "select a.hn,a.an,concat(p.pname,p.fname,' ',p.lname) pname,a.age_y,a.regdate,a.dchdate,a.pdx,i.name diag, a.income 
                    from an_stat a 
 ...
 * 
 */
        $sql5 = "        select a.*, b.an, b.icd10 exd_icd10, b.ename from 
                    (
                      select a.hn,a.an,concat(p.pname,p.fname,' ',p.lname) pname,a.age_y,a.regdate,ip.regtime,a.dchdate,
                             a.pdx,i.name diag , w.`name` ward, a.op0,a.op1, a.op2, a.op3, a.op4, a.op5, a.op6,
                             ic.name icd9n,o.request_date, o.request_time,o.operation_name, d.name statusx, a.income
                      from an_stat a 
                      inner join patient p on p.hn=a.hn
                      left outer join icd101 i on a.pdx=i.code 
	        inner join ward w on a.ward=w.ward
                      inner join ipt ip on ip.an=a.an
	        inner join dchtype d on ip.dchtype=d.dchtype
	        left outer join icd9cm1 ic on ic.`code`=a.op0
	        left outer join operation_list o on a.an=o.an
                      where a.dchdate between   '{$date1}' and '{$date2}'  and a.pdx between 'M170' and 'M179'
                    ) as a
                    left outer join 
                    (
                        select i.an, i.icd10, ip.name ename from iptdiag i inner join icd101 ip on i.icd10=ip.code where i.diagtype='5'
                    ) as b 
                    on a.an=b.an";
        try {
              $rawData5 = \Yii::$app->db1->createCommand($sql5)->queryAll();
        ...
        return $this->render('/site/ipd-ortho/pct/pct2-preview',['names'=>$names,'mText'=>$this->mText,'date1'=>$date1,
                      'date2'=>$date2, 'namet' => $namet,
                      'dataProvider1'=>$dataProvider1,'dataProvider2'=>$dataProvider2,'dataProvider3'=>$dataProvider3,
                      'dataProvider4'=>$dataProvider4,'dataProvider5'=>$dataProvider5
                      ]);             
 ...
 */
        $sql5 = "        select a.*, b.an, b.icd10 exd_icd10, b.ename from 
	        ...
	        left outer join icd9cm1 ic on ic.`code`=a.op0
	        left outer join operation_list o on a.an=o.an
                      where a.dchdate between   '{$date1}' and '{$date2}'  and a.pdx between 'M430' and 'M4399'
                    ) as a
                    left outer join 
                    (
                        select i.an, i.icd10, ip.name ename from iptdiag i inner join icd101 ip on i.icd10=ip.code where i.diagtype='5'
 ...
 */
        $sql5 = "        select a.*, b.an, b.icd10 exd_icd10, b.ename from 
	        ...
	        left outer join icd9cm1 ic on ic.`code`=a.op0
	        left outer join operation_list o on a.an=o.an
                      where a.dchdate between   '{$date1}' and '{$date2}'  and a.pdx between 'M480' and 'M4899'
                    ) as a
                    left outer join 
                    (
                        select i.an, i.icd10, ip.name ename from iptdiag i inner join icd101 ip on i.icd10=ip.code where i.diagtype='5'
   ...
   public function actionPct8_preview($name, $d1, $d2) {
       ...
       try {
        ...
        $sql5 = "select a.*, b.an, b.icd10 exd_icd10, b.ename from 
	        ...
	        left outer join icd9cm1 ic on ic.`code`=a.op0
	        left outer join operation_list o on a.an=o.an
                      where a.dchdate between  '{$date1}' and '{$date2}'  and ((a.pdx between 'T08' and 'T0819')  
                      or a.pdx like 'T093%' or a.pdx like 'S120%' or (a.pdx between 'S130' and 'S1349') or a.pdx like 'S32%')
                    ) as a
                    left outer join 
                    (
 ...
 */
    ...
    public function actionPct9_preview($name, $d1, $d2) {
       ...
       try {
		...
		left outer join icd9cm1 ic on ic.`code`=a.op0
		left outer join operation_list o on a.an=o.an
		where a.dchdate between  '{$date1}' and '{$date2}' and a.pdx between 'S720' and 'S7229'
                    ) as a
                    left outer join 
                    (
                        select i.an, i.icd10, ip.name ename from iptdiag i inner join icd101 ip on i.icd10=ip.code where i.diagtype='5'
