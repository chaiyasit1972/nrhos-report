...
class CopdController extends Controller
{
    ...
    public function actionIndex() {
        $names="คลินิกผู้ป่วยโรคเรื้อรัง(COPD)"; 
         return $this -> render('/site/copd/index',['mText'=>$this->mText,'sText' => $this->sText,'names'=>$names]);
    } 
     public function actionCopdIndex() {
        $names="รายงานทะเบียนผู้ป่วยคลินิก COPD";
        $model = new Formmodel();
        if($model->load(Yii::$app->request->post())){
               $date1 = $model->date1;
               $date2 = $model->date2;
            $url="http://192.168.3.8/stimulrep/stimulsoft/index.php?stimulsoft_client_key=ViewerFx&stimulsoft_report_key=copd/";   
            return $this->redirect($url.'copd1.mrt&d1='.$date1.'&d2='.$date2);                 
        }
            return $this -> render('/site/copd/copd-index',['mText'=>$this->mText,'names'=>$names, 'sText' => $this->sText,
                                     'model' => $model]);
     }    
     ...
     public function actionCopd1Index() {
        ...
        if($model->load(Yii::$app->request->post())){
            ...
            return $this -> render('/site/copd/copd1-index',['mText'=>$this->mText,  'sText' => $this->sText,
...
*/       
       ...
       $sql6="select pname,sum(total) total,sum(Oct) Oct,sum(Nov) Nov,sum(Dece) Dece,sum(Jan) Jan,sum(Feb) Feb,sum(Mar) Mar,
        ...
        return $this -> render('/site/copd/copd1-preview',['dataProvider' => $dataProvider, 'names' => $names,
                                       'mText' => $this->mText,'sText' => $this->sText, 'date1' => $date1, 'date2' => $date2,'yrs' => $yrs]);          
        
    }
     public function actionCopd2Index() {
        $names="รายงานการสูบบุหรี่ในผู้ป่วย COPD (ICD-10 : J44 - J449)";
        $model = new Formmodel();
        if($model->load(Yii::$app->request->post())){
               $date1 = $model->date1;
               $date2 = $model->date2;
            $url="http://192.168.3.8/stimulrep/stimulsoft/index.php?stimulsoft_client_key=ViewerFx&stimulsoft_report_key=copd/";   
            return $this->redirect($url.'copd2.mrt&d1='.$date1.'&d2='.$date2);                 
        }
            return $this -> render('/site/copd/copd2-index',['mText'=>$this->mText,'names'=>$names, 'sText' => $this->sText,
                                     'model' => $model]);
     }  
     public function actionCopd3Index() {
        $names="รายงานสรุปผลการเลิกสูบบุหรี่ในผู้ป่วย COPD (ICD-10 : J44 - J449) สูบแล้วเลิก";
        $model = new Formmodel();
        if($model->load(Yii::$app->request->post())){
               $date1 = $model->date1;
               $date2 = $model->date2;
            $url="http://192.168.3.8/stimulrep/stimulsoft/index.php?stimulsoft_client_key=ViewerFx&stimulsoft_report_key=copd/";   
            return $this->redirect($url.'copd3.mrt&d1='.$date1.'&d2='.$date2);                 
        }
            return $this -> render('/site/copd/copd2-index',['mText'=>$this->mText,'names'=>$names, 'sText' => $this->sText,
                                     'model' => $model]);
     }    
     public function actionCopd4Index() {
        $names="รายงานการรับบริการผู้ป่วย COPD (ICD-10 : J440,J441,J449) อายุ 65 ปีขึ้นไป";
        $model = new Formmodel();
        if($model->load(Yii::$app->request->post())){
               $date1 = $model->date1;
               $date2 = $model->date2;
            $url="http://192.168.3.8/stimulrep/stimulsoft/index.php?stimulsoft_client_key=ViewerFx&stimulsoft_report_key=copd/";   
            return $this->redirect($url.'copd4.mrt&d1='.$date1.'&d2='.$date2);                 
        }
            return $this -> render('/site/copd/copd2-index',['mText'=>$this->mText,'names'=>$names, 'sText' => $this->sText,
                                     'model' => $model]);
     }        
     public function actionCopd5Index() {
        $names="รายงานจำนวนผู้ป่วย COPD (ICD-10 : J440 - J449) รับบริการ OPD";
        $model = new Formmodel();
        if($model->load(Yii::$app->request->post())){
               $date1 = $model->date1;
               $date2 = $model->date2;
            $url="http://192.168.3.8/stimulrep/stimulsoft/index.php?stimulsoft_client_key=ViewerFx&stimulsoft_report_key=copd/";   
            return $this->redirect($url.'copd5.mrt&d1='.$date1.'&d2='.$date2);                 
        }
            return $this -> render('/site/copd/copd2-index',['mText'=>$this->mText,'names'=>$names, 'sText' => $this->sText,
                                     'model' => $model]);
     } 
     public function actionCopd6Index() {
        $names="รายงานจำนวนผู้ป่วย COPD (ICD-10 : J440 - J449) รับบริการ ER";
        $model = new Formmodel();
        if($model->load(Yii::$app->request->post())){
               $date1 = $model->date1;
               $date2 = $model->date2;
            $url="http://192.168.3.8/stimulrep/stimulsoft/index.php?stimulsoft_client_key=ViewerFx&stimulsoft_report_key=copd/";   
            return $this->redirect($url.'copd6.mrt&d1='.$date1.'&d2='.$date2);                 
        }
            return $this -> render('/site/copd/copd2-index',['mText'=>$this->mText,'names'=>$names, 'sText' => $this->sText,
                                     'model' => $model]);
     }      
