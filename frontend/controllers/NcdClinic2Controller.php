...
class NcdClinic2Controller extends Controller
{
    ...
    public function actionClinic8Index() {
        ...
        return $this -> render('/site/ncd/clinic8-index',['mText'=>$this->mText,'sText'=>$this->sText, 'names'=>$names, 'model' => $model]);            
    }
    public function actionClinic9Index() {
        ...
        if($model->load(Yii::$app->request->post())){
            ...
            return $this -> render('/site/ncd/clinic9-index',['mText'=>$this->mText,'sText'=>$this->sText,'names'=>$names,'model'=>$model]);
    } 
    ...
    public function actionClinic10Index() {
        ...
        if($model->load(Yii::$app->request->post())){
            ...
            return $this -> render('/site/ncd/clinic10-index',['mText'=>$this->mText,'sText'=>$this->sText,'names'=>$names,'model'=>$model]);
    }     
    ...
    public function actionClinic11Index() {
        ...
        if($model->load(Yii::$app->request->post())){
            ...
            return $this -> render('/site/ncd/clinic11-index',['mText'=>$this->mText,'sText'=>$this->sText,'names'=>$names,'model'=>$model]);
    }
    public function actionClinic11_preview($d1,$d2) {
     ...
     public function actionClinic12Index() {
        ...
        return $this -> render('/site/ncd/clinic12-index',['mText' => $this->mText,'sText'=>$this->sText,'names' => $names,'model' => $model]);         
    }  
    public function actionClinic12_preview($name,$d1,$d2,$s) {
     ...
     public function actionClinic13Index() {
        ...
        if($model->load(Yii::$app->request->post())){
            ...
            return $this -> render('/site/ncd/clinic13-index',['mText'=>$this->mText,'sText'=>$this->sText,'names'=>$names,'model' => $model]);
    }    
     public function actionClinic14Index() {
        ...
        return $this -> render('/site/ncd/clinic14-index',['mText' => $this->mText,'sText'=>$this->sText,'names' => $names,'model' => $model]);         
    }     
    public function actionClinic14_preview($name, $d1, $d2) {
        $names = $name;
        $date1 = $d1;
        $date2 = $d2;
        $sql1 = "select p.cid,v.hn, concat(p.pname, p.fname, ' ', p.lname) pname, s.name sex, v.age_y, p.addrpart, p.moopart,
     ...
     public function actionClinic15Index() {
        ...
        return $this -> render('/site/ncd/clinic15-index',['mText' => $this->mText,'sText'=>$this->sText,'names' => $names,'model' => $model]);         
    }     
     public function actionClinic16Index() {
        ...
        return $this -> render('/site/ncd/clinic16-index',['mText' => $this->mText,'sText'=>$this->sText,'names' => $names,'model' => $model]);         
    }         
     public function actionClinic17Index() {
        ...
        return $this -> render('/site/ncd/clinic17-index',['mText' => $this->mText,'sText'=>$this->sText,'names' => $names,'model' => $model]);         
    }     
    public function actionClinic17_preview($name,$d1,$d2,$s) {
     ...
     public function actionClinic18Index() {
        ...
        return $this -> render('/site/ncd/clinic18',['mText' => $this->mText,'sText'=>$this->sText,'names' => $names,'model' => $model]);         
    }      
     public function actionClinic19Index() {
        $model = new Formmodel();
        $names="รายงานผู้ป่วยเบาหวานที่ได้รับยาฉีด"; 
        if($model->load(Yii::$app->request->post())){
               $date1 = $model->date1;
               $date2 = $model->date2;   
               $url="http://192.168.3.8/stimulrep/stimulsoft/index.php?stimulsoft_client_key=ViewerFx&stimulsoft_report_key=ncd/"; 
               return $this->redirect($url.'ncd19.mrt&d1='.$date1.'&d2='.$date2);                 
        }         
        return $this -> render('/site/ncd/clinic18',['mText' => $this->mText,'sText'=>$this->sText,'names' => $names,'model' => $model]);         
    }   
     public function actionClinic20Index() {
        $model = new Formmodel();
        $names="รายงานผู้ป่วยโรคเรื้อรัง( DM,HT ) ที่ยังไม่ได้ลงทะเบียนในบัญชี 1 "; 
        if($model->load(Yii::$app->request->post())){
               $date1 = $model->date1;
               $date2 = $model->date2;   
               if($model->select1==1){
               ...
               $url="http://192.168.3.8/stimulrep/stimulsoft/index.php?stimulsoft_client_key=ViewerFx&stimulsoft_report_key=ncd/"; 
               return $this->redirect($url.'ncd20.mrt&d1='.$date1.'&d2='.$date2.'&cn='.$clinic);                 
        }         
        return $this -> render('/site/ncd/clinic1-index',['mText' => $this->mText,'sText'=>$this->sText,'names' => $names,'model' => $model]);         
    } 
    
    public function actionKpiIndex() {
        $model = new Formmodel();        
        $names=" รายงานรายละเอีดตัวชี้วัดกระทรวง(แยกปีงบประมาณ)"; 
        if($model->load(Yii::$app->request->post())){
               $select1 = $model->select1;
               return $this->redirect(['kpi-preview', 'name' =>$names, 'select1' =>$select1]);                
        }        
         return $this -> render('/site/ncd/kpi-index',['mText'=>$this->mText,'names'=>$names, 'sText' => $this->sText, 'model' => $model]);
    }         
    public function actionKpiPreview($name,$select1) {
        $names = "รายงานตัวชี้วัดกระทรวง Kpi (ปีงบประมาณ 25" . $select1 . ")" ;
        $file = 'kpi'.$select1;
        return $this -> render('/site/ncd/'.$file,['mText'=>$this->mText,'names'=>$names,'sText' => $this->sText,
                              'select1'=>$select1]);
    }    
